...
<?php if($this->profile->search_results == 'tagged'):
    ...
    window.addEvent('load', function()
    {
       document.id('igallery_search').addEvent('submit', function(event)
       {
          ...
          var newLocation = '<?php echo JURI::root(true); ?>/index.php?option=com_igallery&amp;igid=<?php echo $topCategory->id; ?>&amp;igtype=category&amp;ighidemenu=1&amp;searchAll=1&amp;Itemid=<?php echo igUtilityHelper::getItemid($topCategory->id) ?>&amp;igchild=1&amp;&amp;igtags=' + tagsClean;
...
<div id="igallery_search<?php echo $this->uniqueid; ?>" class="igallery_search">
	<form action="<?php echo JURI::root(true); ?>/index.php" method="post" id="igallery_search">
